...
let mockCompanies = [{ id: 1, name: 'acme', created_at: '2026-02-25T00:00:00Z' }];
...
describe('CompaniesPage', () => {
  ...
  it('opens dialog and adds a new company to the list', async () => {
    ...
    render(<CompaniesPage />, { wrapper: Wrapper });
    const buttonAddCompany = screen.getByRole('button', { name: /add company/i });
    await user.click(buttonAddCompany);

    const inputCompanyName = await screen.findByRole('textbox', { name: /company/i });
    ...
    const buttonInsertCompany = await screen.findByRole('button', { name: /add/i });
